...
export const isChatMember = (chat, userId) => {
  if (!chat?.members?.length) return false;
  const uid = memberIdString(userId);
  if (!uid) return false;
  return chat.members.some((m) => memberIdString(m) === uid);
};

/** In groups with onlyAdminsCanPost, only the creator may create new messages (text, files, forwards). */
export const canPostMessagesInChat = (chat, userId) => {
  if (!chat?.groupChat) return true;
  if (!chat.onlyAdminsCanPost) return true;
  const uid = memberIdString(userId);
  return memberIdString(chat.creator) === uid;
};
